...
const createOrder = async (userId, cartId, shippingAddress, productIds) => {
    try {
      // Kiểm tra giỏ hàng
      ...
      console.log("Các sản phẩm trong giỏ hàng:", cart.products);
      console.log("Sản phẩm được chọn:", productIds);
      ...
      console.log("Sản phẩm hợp lệ được chọn:", selectedProducts);
      ...
      const products = await Promise.all(
        selectedProducts.map(async (item) => {
          ...
          if (!product) {
            throw { status: 404, message: `Không tìm thấy sản phẩm với ID ${item.productId}` };
          }
          return {
      ...
      const shippingFee = totalPrice > 100000 ? 0 : 50;
